...
def define_discriminator(image_shape):
	...
	d = Activation('tanh')(d)
	# fully connected
	# d = Dense(16384, activation='tanh', kernel_initializer=init)(d)
	d = Dense(1, activation='tanh', kernel_initializer=init)(d)
	d = Activation('tanh')(d)
	# patch output
...
def define_generator(image_shape=(size,size,1)):
	# weight initialization
	init = RandomNormal(stddev=0.02)
	# image input
	...
	d2 = decoder_block(d1, e3, 128, dropout=False)
	d3 = decoder_block(d2, e2, 128, dropout=False)
	d4 = decoder_block(d3, e1, 1, dropout=False)
	# output
...
def summarize_performance(step, g_model, dataset, n_samples=3):
	...
	# save plot to file
	filename1 = 'testing_%06d.png' % (step+1)
	pyplot.savefig(filename1)
	pyplot.close()
	# save the generator model
	filename2 = 'testing_%06d.h5' % (step+1)
...
# load image data
dataset = load_real_samples('drnzbg3.npz')
